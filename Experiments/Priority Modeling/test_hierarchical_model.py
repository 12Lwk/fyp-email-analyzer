...
def predict_priority(subject, message):
    """Predict email priority using hierarchical model"""
    # Load models and scalers
    high_model = xgb.Booster()
    high_model.load_model('Models/high_priority_model.json')
    medium_low_model = xgb.Booster()
    medium_low_model.load_model('Models/medium_low_priority_model.json')
    
    high_scaler = joblib.load('Models/high_priority_scaler.joblib')
    medium_low_scaler = joblib.load('Models/medium_low_priority_scaler.joblib')
    
    # Preprocess email and extract features
    features = preprocess_email(subject, message)
    
    # First level: High vs Rest
    features_scaled = high_scaler.transform(features)
    dmatrix = xgb.DMatrix(features_scaled)
    high_prob = high_model.predict(dmatrix)[0]
    
    if high_prob > 0.4:  # Lowered threshold for High priority
        return 'High', high_prob, [0.0, 0.0, high_prob]
    
    # Calculate Low priority indicators
    informational_score = features[0, 13] * 3.0  # Increased weight
    social_score = features[0, 14] * 2.5         # Increased weight
    optional_score = features[0, 15] * 2.0
    
    # Calculate urgency and action indicators
    ...
    ) / 9.5  # Normalize to 0-1 range
    
    # Second level: Medium vs Low
    features_scaled = medium_low_scaler.transform(features)
    dmatrix = xgb.DMatrix(features_scaled)
    medium_prob = medium_low_model.predict(dmatrix)[0]
    
    # Adjust probabilities based on scores
    if low_priority_score > 0.4:  # Lower threshold for Low priority
        medium_prob = medium_prob * (1 - low_priority_score)
    
    if medium_priority_score > 0.3:  # Lower threshold for Medium priority
        medium_prob = medium_prob * (1 + medium_priority_score)
    
    # Final decision
    if medium_prob > 0.65:  # Slightly increased threshold
        return 'Medium', medium_prob, [0.0, medium_prob, 0.0]
    else:
        low_confidence = max(1 - medium_prob, low_priority_score)
        return 'Low', low_confidence, [low_confidence, 0.0, 0.0]
