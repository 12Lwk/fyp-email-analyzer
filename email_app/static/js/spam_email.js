...
function createEmailRow(email) {
    ...
    row.addEventListener('click', () => {
        window.location.href = `/spam_email_detail/${encodeURIComponent(email.id)}`;
    });
...
async function loadEmails() {
    try {
        ...
        const searchQuery = document.getElementById('emailSearch').value;
        
        console.log('Fetching spam emails from server...');
        const response = await fetch(`/api/emails/spam/?page=${currentPage}&page_size=${pageSize}&search=${encodeURIComponent(searchQuery)}`);
        ...
        if (data.status === 'success' && Array.isArray(data.emails)) {
            console.log('Total spam emails received:', data.emails.length);
            ...
            document.getElementById('emailCount').textContent = `${data.total} emails`;
