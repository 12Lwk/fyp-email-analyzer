...
class EnhancedEmailCategorizationSystem:
    ...
    def extract_email_features(self, subject, message):
        ...
        formality_score = sum(1 for phrase in self.formal_phrases if phrase in message.lower())
        ...
        has_policy_update = int(any(re.search(pattern, message.lower()) for pattern in self.policy_patterns))
        is_internal = sum(1 for marker in self.internal_markers if marker in message.lower())
        
        # Contextual features
        contextual_features = self.extract_contextual_features(subject, message)
...
def main():
    ...
    config = {
        ...
        'model_save_path': 'enhanced_email_categorization_model_v2.joblib',
        'batch_size': 500,
        'test_size': 0.2,
        'random_state': 42
    }
